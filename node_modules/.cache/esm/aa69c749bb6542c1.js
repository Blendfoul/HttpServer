let http,fs,url,os;_bb6‍.w("http",[["default",["http"],function(v){http=v}]]);_bb6‍.w("fs",[["default",["fs"],function(v){fs=v}]]);_bb6‍.w("url",[["default",["url"],function(v){url=v}]]);_bb6‍.w("os",[["default",["os"],function(v){os=v}]]);
...
const httpserver = http.createServer((req, res) => {
    ...
    if(req.method === "GET"){
        ...
        fs.writeFile('./data/online.json', stringed, 'utf-8', error => {if(error)_bb6‍.g.console.error(error)});
        ...
        fs.readFile(fileName, {encoding:'utf-8', flag: 'r'}, (error, data) => {
            if(error){
                ...
                res.end(`<h1>Error getting the file:</h1><bold>${error}</bold>.`);
            }
            else{
                res.writeHead(200, 
                    {
        ...
        _bb6‍.g.console.info('[' + date.getFullYear() +'-' + 
                    (date.getMonth() + 1) + '-' + date.getDate() + ' ' +  
                    date.getHours() + ':'+ date.getMinutes() +  ':'+ date.getSeconds() + 
        ...
        if('/data' == pathName){
            ...
            req.on('end', () => {
                ...
                fs.writeFile('./data/mesa_' + postData.fileName + '.json', jsonContent, 'utf-8', error => {if(error)_bb6‍.g.console.error(error)});
...
Object.keys(ipAdress).forEach(ifName =>{
    ...
    ipAdress[ifName].forEach(iface =>{
        ...
        if(alias >= 1)
            _bb6‍.g.console.log(ifName + ':' + alias, iface.address);
        else
           ip = iface.address;
        ++alias;
    })
...
_bb6‍.g.console.info("Http web server listening at " + ip + ':' + port + '.');